...
test('controller hands and lowered gun preserve spatial ownership and deliberate reload',async()=>{const g=await harness();try{
 assert.equal(g.state().inputController,g.controllers[1]);assert.equal(g.hands.length,2);assert.ok(g.hands.every(h=>h.el.object3D.visible&&h.el.object3D.children.length===1));g.aim();assert.ok(Math.abs(g.rayDirection.y-Math.sin(-23*Math.PI/180))<1e-6);
...
test('fixed -23 degree aim and gun position stay on the selected grip',async()=>{const g=await harness();try{g.grips[1].position.set(.2,1.4,-.4);g.grips[1].rotation.set(.1,.6,.2);g.tick();assert.ok(Math.abs(g.state().gunPitch+23*Math.PI/180)<1e-12);g.aim();const old=g.rayDirection.clone();onkeydown({key:'g',code:'KeyG'});g.tick();g.aim();assert.ok(old.angleTo(g.rayDirection)<1e-7);assert.equal(g.gun.parent,g.grips[1]);onkeydown({key:'h',code:'KeyH'});g.tick();assert.equal(g.gun.parent,g.grips[0]);assert.equal(g.state().holding,0);}finally{g.restore();}});
