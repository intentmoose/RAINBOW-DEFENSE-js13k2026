...
function updateBeam(dt,time){shotClock=Math.max(0,shotClock-dt);flash=Math.max(0,flash-dt);const before=weapon.primed&&!weapon.assist;
 ...
 if(firing){aim();let best=-1,distance=36;const damage=1+(d.gunLevels[mode]-1)*.35;
  ...
  shotClock=mode===0?.18:mode===1?.7:1.2;flash=mode===0?.055:.1;kick=mode===1?.055:.035;tone([520,180,980][mode],.065,[.12,.22,.28][mode],.12,0,"sawtooth");tone(1600,.02,.025,.1,0,"square");twinkle(.5);if(mode===0)weapon.stop();}
 const buildAim=active&&d.state===0&&renderer.xr.isPresenting;if(buildAim){const hit=pick();beamEnd.copy(rayOrigin).addScaledVector(rayDirection,12);if(hit)beamEnd.copy(hit.point);}beamMesh.count=buildAim?1:6;beamMesh.visible=!!(firing||flash||buildAim);if(beamMesh.visible){emitter.getWorldPosition(muzzle);beamRight.set(1,0,0).applyQuaternion(quaternion);
  for(let i=0;i<beamMesh.count;i++){const spread=buildAim?0:(i-2.5)*(mode===1?.45:.012);beamMid.copy(muzzle).addScaledVector(beamRight,mode===1&&!buildAim?(i%2?.035:-.035):spread);point.copy(beamEnd).addScaledVector(beamRight,spread).sub(beamMid);const length=point.length();worldDummy.position.copy(beamMid).addScaledVector(point,.5);worldDummy.quaternion.setFromUnitVectors(up,point.normalize());worldDummy.scale.set(buildAim?.3:mode===2?.6:1,mode===0&&!buildAim?Math.min(length,1.2):length,1);worldDummy.updateMatrix();beamMesh.setMatrixAt(i,worldDummy.matrix);}beamMesh.instanceMatrix.needsUpdate=true;}
 }
